# alpine 3.19 버전의 리룩스를 구축하는데, 파이썬 버전은 3.11로 설치된 이미지를 불러와줘
# alpine - 경령화된 리눅스 버전 => 가볍다
# => 빌드가 계속 반복되는데, 이미지 자체가 무거우면 빌드 속도가 느려짐
...
# 도커 컨테이너에서 어떤 일이 벌어지고 있는지 알아야 디버깅을 할 수 있다
# 실시간으로 볼 수 있기 때문에 컨테이너 관리가 편해집니다.
ENV PYTHONUNBUFFERED 1

# tmp에 넣은 이유: 컨테이너를 최대한 경량상태로 유지하기 위해
